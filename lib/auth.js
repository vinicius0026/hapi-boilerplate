...
internals.setupAuth = function (server, next) {
  // TODO: refactor options to a config file
  server.auth.strategy('session', 'cookie', {
    password: 'superlongandsecretpasswordthatnoonewillguess',
    cookie: 'hapi-cookie',
    ttl: 24 * 60 * 60 * 1000,
    isSecure: false
  })
  ...
  console.log('setting auth routes')
  ...
  server.route([
    {
      ...
      config: {
        ...
        handler: (request, reply) => {
          internals.getValidatedUser(request.payload.username, request.payload.pasword)
            .then(user => {
              if (!user) {
